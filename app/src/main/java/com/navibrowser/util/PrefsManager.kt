...
class PrefsManager(context: Context) {
    ...
    /** 页面加载完成后若存在已保存凭据则自动填充（无需点击 FAB）。 */
    var autoFillOnLoadEnabled: Boolean
        get() = prefs.getBoolean("password_autofill_on_load", false)
        set(v) = prefs.edit { putBoolean("password_autofill_on_load", v) }
    /** 保存密码时若与已保存的不同，提示“更新”而非静默覆盖。 */
    var passwordUpdatePromptEnabled: Boolean
        get() = prefs.getBoolean("password_update_prompt", true)
        set(v) = prefs.edit { putBoolean("password_update_prompt", v) }
