...
class PasswordManagerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val settingsSection = findViewById<LinearLayout>(R.id.passwordSettingsSection)
        val switchAccessAuth = findViewById<SwitchMaterial>(R.id.switchAccessAuth)
        val switchFillAuth = findViewById<SwitchMaterial>(R.id.switchFillAuth)

        rv.layoutManager = LinearLayoutManager(this)

        switchAccessAuth.isChecked = viewModel.prefs.passwordAccessAuthEnabled
        switchFillAuth.isChecked = viewModel.prefs.passwordFillAuthEnabled
        switchAccessAuth.setOnCheckedChangeListener { _, isChecked ->
            viewModel.prefs.passwordAccessAuthEnabled = isChecked
        }
        switchFillAuth.setOnCheckedChangeListener { _, isChecked ->
            viewModel.prefs.passwordFillAuthEnabled = isChecked
        }
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.passwordRepo.passwords.combine(queryFlow) { list, q ->
                    ...
                    rv.adapter = PasswordListAdapter(
                        ...
                        onCopyUsername = {
                            if (!authenticated && viewModel.prefs.passwordAccessAuthEnabled) {
                                requireAuth { copyToClipboard(this@PasswordManagerActivity, "账号", it.username) }
                            } else {
                                copyToClipboard(this@PasswordManagerActivity, "账号", it.username)
                            }
                        },
                        onCopyPassword = {
                            val pwd = decryptPassword(it)
                            if (!authenticated && viewModel.prefs.passwordAccessAuthEnabled) {
                                requireAuth { copyToClipboard(this@PasswordManagerActivity, "密码", pwd) }
                            } else {
        ...
        if (viewModel.prefs.passwordAccessAuthEnabled) {
    ...
    private fun requireAuth(onSuccess: () -> Unit) {
        BiometricAuthUtil.authenticate(this,
            "验证身份",
            "验证后才能访问密码管理器",
            onSuccess = { authenticated = true; onSuccess() },
            onError = { if (!authenticated) finish() }
        )
    }
    ...
    private fun showViewDialog(item: SavedPassword) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("复制密码") { _, _ ->
                if (!authenticated && viewModel.prefs.passwordAccessAuthEnabled) {
                    requireAuth { copyToClipboard(this, "密码", password) }
                } else {
