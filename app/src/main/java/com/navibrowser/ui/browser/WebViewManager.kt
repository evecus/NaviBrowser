...
class WebViewManager(private val context: Context) {
    ...
    var onVideoDomFound: ((VideoSniffer.SniffedVideo) -> Unit)? = null
    var onGestureAction: ((action: Int) -> Unit)? = null
    var onOpenSearchEngineSettings: (() -> Unit)? = null
    /** 页面开始加载时触发，传入 url，供调用方注入 document-start 脚本/视频嗅探 JS。 */
    var onPageStartedUrl: ((String) -> Unit)? = null
    ...
    private fun buildWebView(tab: Tab): WebView {
        ...
        // 视频嗅探 JS 桥接：页面内 DOM/媒体源扫描结果回调
        if (prefs.videoSnifferEnabled) {
            wv.addJavascriptInterface(
                VideoScannerJs { url, mime ->
                    val title = currentTab?.info?.title ?: wv.url?.let { UrlUtils.getDomain(it) } ?: ""
                    // 通过 recordDom 入库（含去重）后再回调 UI
                    val before = VideoSniffer.getAll(tab.id).size
                    VideoSniffer.recordDom(url, title, mime, tab.id)
                    if (VideoSniffer.getAll(tab.id).size > before) {
                        onVideoDomFound?.invoke(VideoSniffer.SniffedVideo(url, title, mime))
                    }
                }, "__naviVideoScanner"
            )
        }
        ...
        wv.webViewClient = object : WebViewClient() {
            override fun onPageStarted(view: WebView, url: String?, favicon: Bitmap?) {
                ...
                VideoSniffer.clear(tab.id)
                // document-start 注入：油猴脚本 + 视频嗅探 DOM 监听
                if (url != null && url.startsWith("http")) {
                    onPageStartedUrl?.invoke(url)
                    view.evaluateJavascript(VIDEO_SNIFFER_JS, null)
                }
            }
            ...
            override fun onPageFinished(view: WebView, url: String?) {
                ...
                url?.let {
                    ...
                    if (prefs.searchEngineQuickSwitchEnabled)
                        view.evaluateJavascript(SearchEngineSwitcher.buildInjectionJs(prefs), null)
                }
            ...
            override fun shouldOverrideUrlLoading(view: WebView, request: WebResourceRequest): Boolean {
                ...
                if (url == "navi://switcher-settings") {
                    onOpenSearchEngineSettings?.invoke()
                    return true
                }
            ...
            override fun shouldInterceptRequest(view: WebView, request: WebResourceRequest): WebResourceResponse? {
                ...
                if (prefs.videoSnifferEnabled) {
                    ...
                    VideoSniffer.sniff(request, title, tab.id)
                }
                return null
            }
